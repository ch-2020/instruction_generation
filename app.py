...
def main():
    ...
    with st.sidebar:
        ...
        component_type = st.selectbox(
            ...
            help="Select the type of component you want to disassemble. Choose 'Auto-detect' to let the AI determine the component type from the document."
        )
        
        if component_type == "Other":
            custom_component = st.text_input(
                "Specify Component",
                placeholder="e.g., Door Handle Assembly, Dashboard Module",
                help="Enter the specific component you want to disassemble"
            )
        else:
                ...
                if st.button("🚀 Generate PDF Instructions", type="primary"):
                    with st.spinner("Processing PDF and generating instructions..."):
                        try:
                            # Process PDF
                            result = asyncio.run(
                                generator.generate_instructions_from_pdf(
                                    tmp_path,
